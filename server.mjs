...
const
  ...
  server = createServer(serve(async (request, response) => {
    ...
    const
      ...
      //например при /dsds-forum функция getAllPosts не запустится
      arrRequestUrl = request.url.split('/'),
      posts = await checkForumURL(arrRequestUrl),
...
async function checkForumURL(arr) {
  let posts = [];
